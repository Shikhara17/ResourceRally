...
public class PEAgent extends Agent {
    ...
    public Map<Integer, Action> middleStep(State.StateView stateView, History.HistoryView historyView){
        ...
        if (!plan.empty()) {
            ...
            for(Integer j : peasantsToRemove){
                thisAction.setPeasantsActing(j);
            }

            if (thisAction.getPeasantsActing().isEmpty()) {
                plan.pop();
            }
            ...
            if (!plan.empty()) {
                ...
                if (action instanceof BuildPeasant){
                    ...
                    for(Integer i : action.getPeasantsActing()) {
    ...
    /**
     * Method that can be used to fetch Peasant SEPIA id in every turn.
     * @param stateView View of the Current State
     * 
     */
    public void getSEPIAID(State.StateView stateView){
        for (int unitId : stateView.getUnitIds(playernum)) {
            Unit.UnitView unit = stateView.getUnit(unitId);
            String unitType = unit.getTemplateView().getName().toLowerCase();
    ...
    private Action createSepiaAction(StripsAction action) {
        return action.createSepiaAction(currentPeasantId);
    }
