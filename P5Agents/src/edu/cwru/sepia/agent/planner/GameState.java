...
public class GameState implements Comparable<GameState> {
    ...
    public HashMap<Integer, Peasant> peasants = new HashMap<>();
    ...
    //number of peasants in current state
    public int numberOfPeasants;

    //helper methods to clone resources and peasants
    public HashMap<Integer, Resource> getResources() {
        HashMap<Integer, Resource> newMap = new HashMap<>();
        this.resources.forEach((k, v) -> {
            newMap.put(k, v.resourceClone());
        });
        return newMap;
    }
    ...
    private void initializeUnits(int playernum) {
        ...
        for (int id : state.getUnitIds(playernum)) {
            ...
            if (unitName.equals("townhall")) {
                ...
                this.peasants.put(1, new Peasant(id, peasantPosition, false, 0, 0));
    ...
    public GameState(GameState parent,double cost, HashMap<Integer, Peasant> peasants,int goldDeposited,int woodDeposited, HashMap<Integer, Resource> resources, StripsAction action) {
    ...
    public static class Peasant {
        ...
        public Peasant clone() {
            Peasant newPeasant = new Peasant(id, position, isCarrying, goldCarrying, woodCarrying);
            newPeasant.currentResourceType = currentResourceType;
            return newPeasant;
        }
    }
    ...
    public List<GameState> generateChildren() {
        ...
        for (Resource resource : getAllResources().values()) {
            ...
            //As we have multiple peasants and all available/possible peasants must act in a single state and generate a single child state we iterate through all the possibilities of STRIPSactions
            for (int k = 1; k <= numberOfPeasants; k++) {
                ...
                if (this.currentGold < requiredGold && resource.resourceType == ResourceNode.Type.GOLD_MINE) {
                	...
                	moveToResource = new MoveTo(resource.resourcePosition, this.peasants, k);
                    //here the StripsAction that we generate is for harvesting gold. hence the last parameter(isGold) is set to true
                    harvestAtResource = new Harvest(new Position(townhallStateView.getXPosition(), townhallStateView.getYPosition()), resource.resourcePosition, k, resource.resourceId, true);
...
//                else if (this.requiredWood > 0 && resource.resourceType == ResourceNode.Type.TREE) {


                	moveToResource = new MoveTo(resource.resourcePosition, this.peasants, k);           	
                	//here the StripsAction that we generate is for harvesting wood. hence the last parameter(isGold) is set to false
                	harvestAtResource =   new Harvest(new Position(townhallStateView.getXPosition(), townhallStateView.getYPosition()), resource.resourcePosition, k, resource.resourceId, false);
            ...
            for (int k = 1; k <= numberOfPeasants; k++) {
                ...
                MoveTo movetoTownhall = new MoveTo(townhallPosition, this.peasants, k);
                

                Deposit deposit = new Deposit(resource.resourcePosition, townhallPosition, k , townhallStateView.getID());
    ...
    public double heuristic() {
		...
		//cumulate all the resources being carried by peasants
		for (Peasant p : peasants.values()) {
			if (p.isCarrying) {
				goldCarrying += p.goldCarrying;
				woodCarrying += p.woodCarrying;
			}
		}
		//amount of gold that is still required to reach goal state from current state. 
		int remainingGoldCost = requiredGold - currentGold - goldCarrying;
		//amount of wood that is still required to reach goal state from current state.
		int remainingWoodCost = requiredWood - currentWood - woodCarrying;
		
		//diving it my number of peasants because states where multiple peasants act help in reaching solution faster
		return (remainingGoldCost+remainingWoodCost)/numPeasants;
    ...
    public int hashCode() {
		...
		int heldGold = 0;
		for (Peasant p : peasants.values()) {
			heldWood += p.goldCarrying;
			heldGold += p.woodCarrying;
			hash *= 31 + p.position.hashCode();
		}
